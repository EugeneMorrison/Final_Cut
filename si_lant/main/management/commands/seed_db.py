...
from ...models import DenialType, RecoveryMethod, Claim, Service, ServiceType, Equipment, VehicleModel, EngineModel, TransmissionModel, DriveAxleModel, SteeringAxleModel, ServiceCompany
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        file_path = os.path.join(current_directory + '/main/management/commands/', 'Equipments_lookups.txt')  # Путь к файлу lookups.txt в текущей директории
        ...
        current_directory = os.getcwd()  # Получаем текущую рабочую директорию
        file_path = os.path.join(current_directory + '/main/management/commands/', 'Service.txt')
        if not os.path.isfile(file_path):
            self.stdout.write(self.style.ERROR('Файл не найден'))
            return
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                Service_type_object, _ = ServiceType.objects.get_or_create(
                    name=Service_type,
                ...
                Service, _ = Service.objects.get_or_create(
                    Equipment=Equipment.objects.get(vin=vin),
                    Service_type=Service_type_object,
        ...
        current_directory = os.getcwd()  # Получаем текущую рабочую директорию
        file_path = os.path.join(current_directory + '/main/management/commands/', 'Claim.txt')
        if not os.path.isfile(file_path):
            self.stdout.write(self.style.ERROR('Файл не найден'))
            return
